...
public class ChatRoomService {
    ...
    @Transactional
    public ChatRoomResponse updateChatRoomName(String customName, Long otherMemberId, Long myId, Long roomId) {
    ...
    protected ChatRoomResponse getChatRoomInfoWithUnreadCount(Long myId, Long otherMemberId, ChatRoom chatRoom) {
        ChatRoomResponse dto = chatRepositoryFacade.getChatRoomInfo(myId, otherMemberId, chatRoom);

        int unreadCount = chatRedisService.getUnreadCount(dto.roomId(), myId);

        return ChatRoomResponse.withUnreadCount(dto, unreadCount);
    }
    ...
    public List<ChatRoomResponse> getChatRooms(Long memberId) {
        List<ChatRoomResponse> rooms = chatRepositoryFacade.findChatRoomsByMemberId(memberId);

        return rooms.stream()
                    .map(room -> {
                        int unreadCount = chatRedisService.getUnreadCount(room.roomId(), memberId);
                        return ChatRoomResponse.withUnreadCount(room, unreadCount);
                    })
                    .toList();
    }
